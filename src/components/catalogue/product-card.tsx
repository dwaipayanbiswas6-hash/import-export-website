...
export function ProductCard({ product }: { product: Product }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[2rem] border border-[color:var(--line)] bg-[color:var(--card)] shadow-[0_18px_55px_rgba(53,39,17,.07)] transition duration-300 hover:-translate-y-1 hover:border-[color:var(--gold-soft)] hover:shadow-[0_24px_65px_rgba(53,39,17,.12)]">
      <Link
      ...
      <div className="flex flex-1 flex-col p-6">
        <p className="text-xs font-bold uppercase tracking-[.18em] text-[color:var(--gold-dark)]">
          {product.category}
        </p>
        ...
        <dl className="mt-5 grid gap-3 border-t border-[color:var(--line)] pt-5 text-sm">
          <div>
            ...
            <dd className="mt-1 line-clamp-2 text-[color:var(--muted)]">
        ...
        <div className="mt-auto grid gap-3 pt-6 sm:grid-cols-2">
          <Link
            className="button-secondary min-h-11 px-4 py-2 text-center"
            href={`/products/${product.slug}`}
          >
            View details
          </Link>
          <Link
            className="button-primary min-h-11 px-4 py-2 text-center"
