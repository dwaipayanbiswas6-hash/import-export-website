...
function CallbackContent() {
  ...
  useEffect(() => {
    async function finishSignIn() {
      ...
      if (!supabase) {
        setError('The buyer portal is not configured yet.');
        return;
      }
      ...
      <div className="premium-card w-full text-center">
        {error ? (
          <>
            ...
            <h1 className="text-3xl font-semibold">We could not open the portal.</h1>
            <p className="mt-5 leading-7 text-[color:var(--muted)]">{error}</p>
            <a className="button-primary mt-8" href="/portal/login">
          ...
          <>
            <LoaderCircle
              ...
              size={36}
            />
            <h1 className="mt-6 text-3xl font-semibold">Opening your buyer portal</h1>
            <p className="mt-4 text-[color:var(--muted)]">
              Verifying the secure email link…
            </p>
          </>
        )}
      </div>
